...
void test_kvlist_operations() {
    ...
    assert(kvlist_get_first_key(list) != NULL && "Head should not be NULL after append");
    ...
    assert(kvlist_get_last_kv(list) == pair2 && "Tail should be pair2 after extend");

    kvlist_sort(list);
    assert(strcmp(kvlist_get_first_key(list), "key1") <= 0 && "List should be sorted");
