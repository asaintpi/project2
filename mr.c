...
void init_hash_table() {
    ...
    keys = malloc(10000 * sizeof(char*)); // Allocate space for keys, adjust size as needed
    if (!keys) {
...
void print_hash_table() {
    for (int i = 0; i < num_keys; i++) {
        ...
        ENTRY *found_item;
        if (hsearch_r(item, FIND, &found_item, &ht_tab)) {
            printf("%s: %d\n", found_item->key, ((count_data *)found_item->data)->count);
        }
    }
}
...
void map_reduce(mapper_t mapper, size_t num_mapper, reducer_t reducer,
    ...
    for (size_t i = 0; i < num_mapper; i++) {
        ...
        mapperArgs[i].output = kvlist_new();  // Individual output list for each thread
